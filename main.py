...
# S-boxes (8 S-boxes each with 4x16 table)
S_BOX = [
...
def generate_keys(key: str) -> List[str]:
    ...
    for i in range(20):
        if i % 2 == 0:
            left_half = left_rotate(left_half, ROTATIONS[i])
        else:
            right_half = left_rotate(right_half, ROTATIONS[i])
        
        transformed_left = xor(left_half, left_rotate(left_half, 1))
        transformed_right = xor(right_half, left_rotate(right_half, 1))
        combined_key = transformed_left + transformed_right
...
def custom_feistel(right: str, key: str) -> str:
    expanded_right = permute(right, E)
    xored = xor(expanded_right, key)
    custom_xored = xor(xored, left_rotate(xored, 3))
    substituted = sbox_substitution(custom_xored)
    inverted = ''.join(['1' if bit == '0' else '0' for bit in substituted])
    result = permute(inverted, P)
...
def des_encrypt(plaintext: str, key: str) -> str:
    ...
    for i in range(20):
        new_right = xor(left, custom_feistel(right, keys[i]))
